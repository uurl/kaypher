...
import com.koneksys.kaypher.parser.SqlBaseParser.IntegerLiteralContext;
import com.koneksys.kaypher.parser.SqlBaseParser.IntervalClauseContext;
import com.koneksys.kaypher.parser.SqlBaseParser.LimitClauseContext;
import com.koneksys.kaypher.parser.SqlBaseParser.NumberContext;
import com.koneksys.kaypher.parser.SqlBaseParser.TablePropertiesContext;
import com.koneksys.kaypher.parser.SqlBaseParser.TablePropertyContext;
...
import com.koneksys.kaypher.parser.tree.DescribeFunction;
...
import com.koneksys.kaypher.parser.tree.DropTopic;
import com.koneksys.kaypher.parser.tree.Explain;
...
import com.koneksys.kaypher.parser.tree.HoppingWindowExpression;
...
import com.koneksys.kaypher.parser.tree.ListFunctions;
import com.koneksys.kaypher.parser.tree.ListProperties;
import com.koneksys.kaypher.parser.tree.ListQueries;
import com.koneksys.kaypher.parser.tree.ListRegisteredTopics;
...
import com.koneksys.kaypher.parser.tree.PrintTopic;
...
import com.koneksys.kaypher.parser.tree.Relation;
import com.koneksys.kaypher.parser.tree.RunScript;
...
import com.koneksys.kaypher.parser.tree.SessionWindowExpression;
import com.koneksys.kaypher.parser.tree.SetProperty;
import com.koneksys.kaypher.parser.tree.ShowColumns;
...
import com.koneksys.kaypher.parser.tree.Struct.Builder;
...
import com.koneksys.kaypher.parser.tree.TerminateQuery;
import com.koneksys.kaypher.parser.tree.TimeLiteral;
import com.koneksys.kaypher.parser.tree.TimestampLiteral;
import com.koneksys.kaypher.parser.tree.TumblingWindowExpression;
import com.koneksys.kaypher.parser.tree.Type;
import com.koneksys.kaypher.parser.tree.UnsetProperty;
import com.koneksys.kaypher.parser.tree.WhenClause;
import com.koneksys.kaypher.parser.tree.WindowExpression;
...
import com.koneksys.kaypher.util.Pair;
...
public class AstBuilder extends SqlBaseBaseVisitor<Node> {
  ...
  private StructuredDataSource getDataSourceForAlias(
    ...
    if (leftAliased.getAlias().equalsIgnoreCase(alias.toString())) {
      ...
      throw new KaypherException("Invalid alias used in join: alias='"
  ...
  public Node visitSelectSingle(final SqlBaseParser.SelectSingleContext context) {
    ...
    if (!alias.isPresent()) {
      if (selectItemExpression instanceof QualifiedNameReference) {
        ...
        } else if (dereferenceExpressionString.contains(KaypherConstants.STRUCT_FIELD_REF)) {
          alias = Optional.of(
              replaceDotFieldRef(
  ...
  private int processIntegerNumber(final NumberContext number, final String context) {
    ...
    throw new KaypherException("Value must be integer in for command: " + context);
  }
  ...
  public Node visitJoinRelation(final SqlBaseParser.JoinRelationContext context) {
    if (context.joinCriteria().ON() == null) {
      throw new KaypherException("Invalid join criteria specified. KSQL only supports joining on "
  ...
  public Node visitColumnReference(final SqlBaseParser.ColumnReferenceContext context) {
    ...
    if (dataSourceExtractor.getJoinLeftSchema() != null) {
      if (dataSourceExtractor.getCommonFieldNames().contains(columnName)) {
        throw new KaypherException("Field " + columnName + " is ambiguous.");
      }
  ...
  private static Type getType(final SqlBaseParser.TypeContext type) {
    ...
    if (type.ARRAY() != null) {
      return com.koneksys.kaypher.parser.tree.Array.of(getType(type.type(0)));
    }

    if (type.MAP() != null) {
      return com.koneksys.kaypher.parser.tree.Map.of(getType(type.type(1)));
    }
