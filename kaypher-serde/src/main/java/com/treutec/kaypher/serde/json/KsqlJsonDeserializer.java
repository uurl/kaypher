...
public class KaypherJsonDeserializer implements Deserializer<Object> {
  // CHECKSTYLE_RULES.ON: ClassDataAbstractionCoupling

  private static final Logger LOG = LoggerFactory.getLogger(KaypherJsonDeserializer.class);
  ...
  private static final Map<Schema.Type, Function<JsonValueContext, Object>> HANDLERS = ImmutableMap
      ...
      .put(Type.STRING, KaypherJsonDeserializer::processString)
      .put(Type.ARRAY, KaypherJsonDeserializer::enforceElementTypeForArray)
      .put(Type.MAP, KaypherJsonDeserializer::enforceKeyAndValueTypeForMap)
      .put(Type.STRUCT, KaypherJsonDeserializer::enforceFieldTypesForStruct)
      .put(Type.BYTES, KaypherJsonDeserializer::enforceValidBytes)
      .build();

  private final PersistenceSchema physicalSchema;

  public KaypherJsonDeserializer(
  ...
  private static Object enforceFieldType(
      final KaypherJsonDeserializer deserializer,
      final Schema schema,
  ...
  private static Object enforceFieldType(final JsonValueContext context) {
    ...
    final Function<JsonValueContext, Object> handler = HANDLERS.getOrDefault(
        context.schema.type(),
        type -> {
          throw new KaypherException("Type is not supported: " + type);
        });
  ...
  private static String processString(final JsonValueContext context) {
    if (context.val instanceof ObjectNode) {
      try {
        ...
        throw new KaypherException("Unexpected inability to write value as string: " + context.val);
      }
    }
  ...
  private static Struct enforceFieldTypesForStruct(final JsonValueContext context) {
    ...
    for (Field ksqlField : context.schema.fields()) {
      // the "case insensitive" strategy leverages that all KAYPHER fields are internally
  ...
  private static class JsonValueContext {
    ...
    private final KaypherJsonDeserializer deserializer;
