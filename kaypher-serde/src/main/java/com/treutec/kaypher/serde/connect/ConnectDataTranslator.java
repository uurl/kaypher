...
import io.confluent.ksql.util.KaypherPreconditions;
...
/**
 * Translates full set of Connect types to the limited subset supported by KAYPHER.
 *
 * <p>Responsible for the coercion of connect types to the subset KAYPHER supports and handling
...
public class ConnectDataTranslator implements DataTranslator {
  ...
  @Override
  public Object toKaypherRow(final Schema connectSchema, final Object connectData) {
    if (connectData == null) {
      return null;
    }

    return toKaypherValue(schema, connectSchema, connectData, "");
  }
  ...
  private Object toKaypherValue(
    ...
    // Map a connect value+schema onto the schema expected by KAYPHER. For now this involves:
    ...
    switch (schema.type()) {
      ...
      case BYTES:
        return toKaypherBytes(convertedValue, connectSchema);
      case ARRAY:
        return toKaypherArray(
      ...
      case STRUCT:
        return toKaypherStruct(schema, connectSchema, (Struct) convertedValue, pathStr);
      case STRING:
  ...
  private Object toKaypherBytes(
      final Object convertedValue,
      final Schema schema
  ) {
    KaypherPreconditions.checkArgument(DecimalUtil.isDecimal(schema), "BYTES type must be DECIMAL");
    KaypherPreconditions.checkArgument(convertedValue instanceof BigDecimal,
        "must serialize decimal type as BigDecimal. Got: " + convertedValue.getClass());

    return convertedValue;
  }

  private List<?> toKaypherArray(
  ...
  private Struct toKaypherStruct(
      final Schema schema,
      final Schema connectSchema,
