...
public final class JdbcSource implements SupportedConnector {
  ...
  public Map<String, String> resolveConfigs(final Map<String, String> configs) {
    ...
    if (key != null) {
      resolved.merge(
          "transforms",
          "kaypherCreateKey,kaypherExtractString",
          (a, b) -> String.join(",", a, b));

      resolved.put("transforms.kaypherCreateKey.type", ValueToKey.class.getName());
      resolved.put("transforms.kaypherCreateKey.fields", key);

      resolved.put("transforms.kaypherExtractString.type", ExtractField.Key.class.getName());
      resolved.put("transforms.kaypherExtractString.field", key);
    }
