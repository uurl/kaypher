...
public class UdfLoader {
  ...
  void loadUdfFromClass(final Class<?>... udfClasses) {
    for (final Class<?> theClass : udfClasses) {
      loadUdfFromClass(
          theClass, KaypherScalarFunction.INTERNAL_PATH);
    }
  }
  ...
  void loadUdfFromClass(
    ...
    if (udfDescriptionAnnotation == null) {
      throw new KaypherException(String.format("Cannot load class %s. Classes containing UDFs must"
          + "be annotated with @UdfDescription.", theClass.getName()));
    }
    final String functionName = udfDescriptionAnnotation.name();
    final String sensorName = "kaypher-udf-" + functionName;
  ...
  private Function<KaypherConfig, Kudf> getUdfFactory(
      final Method method,
      final UdfDescription udfDescriptionAnnotation,
    ...
    return kaypherConfig -> {
      ...
      if (actualUdf instanceof Configurable) {
        ((Configurable) actualUdf)
            .configure(kaypherConfig.getKaypherFunctionsConfigProps(functionName));
      }
