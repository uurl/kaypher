...
class UdafFactoryInvoker implements FunctionSignature {
  ...
  UdafFactoryInvoker(
    ...
    if (!(Udaf.class.equals(method.getReturnType())
      ...
      throw new KaypherException("UDAFs must implement " + Udaf.class.getName() + " or "
          + TableUdaf.class.getName() + ". "
          + functionInfo);
    }
    if (!Modifier.isStatic(method.getModifiers())) {
      throw new KaypherException("UDAF factory methods must be static " + method);
    }
  ...
  KaypherAggregateFunction createFunction(final AggregateFunctionInitArguments initArgs) {
    final Object[] factoryArgs = initArgs.args().toArray();
    try {
      final Udaf udaf = (Udaf)method.invoke(null, factoryArgs);
      final KaypherAggregateFunction function;
      ...
      throw new KaypherException("Failed to invoke UDAF factory method", e);
    }
  }
