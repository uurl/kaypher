...
public class ArrayContainsKudf implements Kudf {
  ...
  public Object evaluate(final Object... args) {
    if (args.length != 2) {
      throw new KaypherFunctionException("ARRAY_CONTAINS udf should have two input argument. "
    ...
    throw new KaypherFunctionException("Invalid type parameters for " + Arrays.toString(args));
  }
  ...
  private boolean jsonStringArrayContains(final Object searchValue, final String jsonArray) {
    ...
    try (JsonParser parser = JSON_FACTORY.createParser(jsonArray)) {
      ...
      throw new KaypherException("Invalid JSON format: " + jsonArray, e);
    }
    return false;
  }
  ...
  private JsonToken getType(final Object searchValue) {
    ...
    throw new KaypherFunctionException("Invalid Type for search value " + searchValue);
  }
}
