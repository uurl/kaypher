 ...
 * and not a kaypher reserved word and is a valid java identifier.
 */
@ThreadSafe
class FunctionNameValidator implements Predicate<String> {
  ...
  private static final Set<String> ALLOWED_KAYPHER_WORDS
      = ImmutableSet.copyOf(Arrays.asList("concat", "substring"));

  private static final Set<String> KAYPHER_RESERVED_WORDS = createFromVocabulary();
  ...
  public boolean test(final String functionName) {
    if (functionName == null
        ...
        || KAYPHER_RESERVED_WORDS.contains(functionName.toLowerCase())) {
      return false;
    }
  ...
  private static Set<String> createFromVocabulary() {
    ...
    for (int i = 0; i < tokens; i++) {
      ...
      if (symbolicName != null) {
        ...
        if (!ALLOWED_KAYPHER_WORDS.contains(keyWord)) {
          builder.add(keyWord);
        }
      }
    }
