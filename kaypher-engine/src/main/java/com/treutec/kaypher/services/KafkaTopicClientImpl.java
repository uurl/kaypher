...
import com.treutec.kaypher.exception.KafkaDeleteTopicsException;
...
import com.treutec.kaypher.topic.TopicProperties;
...
public class KafkaTopicClientImpl implements KafkaTopicClient {
  ...
  public void createTopic(
    ...
    try {
      ...
      throw new KaypherTopicAuthorizationException(
  ...
  private short getDefaultClusterReplication() {
    try {
      ...
      throw new KaypherServerException("Could not get default replication from Kafka cluster!", e);
    }
  }
  ...
  public Set<String> listNonInternalTopicNames() {
    return listTopicNames().stream()
        .filter((topic) -> !(topic.startsWith(KaypherConstants.KAYPHER_INTERNAL_TOPIC_PREFIX)
            || topic.startsWith(KaypherConstants.CONFLUENT_INTERNAL_TOPIC_PREFIX)))
  ...
  public TopicCleanupPolicy getTopicCleanupPolicy(final String topicName) {
    ...
    switch (policy) {
      ...
      default:
        throw new KaypherException("Could not get the topic configs for : " + topicName);
    }
  }
  ...
  private static boolean isInternalTopic(final String topicName, final String applicationId) {
    return topicName.startsWith(applicationId + "-")
        && (topicName.endsWith(KaypherConstants.STREAMS_CHANGELOG_TOPIC_SUFFIX)
        || topicName.endsWith(KaypherConstants.STREAMS_REPARTITION_TOPIC_SUFFIX));
  }
