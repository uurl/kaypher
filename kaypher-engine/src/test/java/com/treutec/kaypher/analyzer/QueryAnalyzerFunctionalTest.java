...
import static com.treutec.kaypher.util.ExpressionMatchers.qualifiedNameExpressions;
...
public class QueryAnalyzerFunctionalTest {
  ...
  public void shouldCreateAnalysisForSimpleQuery() {
    ...
    assertThat(fromDataSource.getDataSource(), instanceOf(KaypherStream.class));
  ...
  public void shouldCreateAnalysisForCsas() {
    // Given:
    final PreparedStatement<CreateStreamAsSelect> statement = KaypherParserTestUtil.buildSingleAst(
  ...
  public void shouldCreateAnalysisForCtas() {
    // Given:
    final PreparedStatement<CreateTableAsSelect> statement = KaypherParserTestUtil.buildSingleAst(
    ...
    final AliasedDataSource fromDataSource = analysis.getFromDataSources().get(0);
    assertThat(fromDataSource.getDataSource(), instanceOf(KaypherTable.class));
  ...
  public void shouldCreateAnalysisForInsertInto() {
    // Given:
    final PreparedStatement<InsertInto> statement = KaypherParserTestUtil.buildSingleAst(
    ...
    assertThat(into.getKaypherTopic(), is(test0.getKaypherTopic()));
  }
  ...
  public void shouldAnalyseWindowedAggregate() {
    ...
    assertThat(aggregateAnalysis.getFinalSelectExpressions(), equalTo(Arrays.asList(ITEM_ID, new ColumnReferenceExp(
        ColumnRef.withoutSource(ColumnName.of("KAYPHER_AGG_VARIABLE_0"))))));
  ...
  public void shouldProcessHavingExpression() {
    ...
    assertThat(havingExpression, equalTo(new ComparisonExpression(
        ...
        new ColumnReferenceExp(ColumnRef.withoutSource(ColumnName.of("KAYPHER_AGG_VARIABLE_1"))),
  ...
  public void shouldHandleValueFormat() {
    ...
    // Then:
    assertThat(analysis.getInto().get().getKaypherTopic().getValueFormat().getFormat(),
        is(Format.DELIMITED));
  }

  private Query givenQuery(final String sql) {
    return KaypherParserTestUtil.<Query>buildSingleAst(sql, metaStore).getStatement();
  }
}