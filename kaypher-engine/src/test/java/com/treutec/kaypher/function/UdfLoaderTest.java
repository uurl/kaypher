...
import static com.treutec.kaypher.util.KaypherConfig.KAYPHER_FUNCTIONS_PROPERTY_PREFIX;
...
import com.treutec.kaypher.function.udaf.TestUdaf;
...
/**
 * This uses kaypher-engine/src/test/resource/udf-example.jar to load the custom jars.
...
public class UdfLoaderTest {
  ...
  @Test
  public void shouldLoadFunctionsInKaypherEngine() {
    ...
    final Kudf substring1 = function.getFunction(
        Arrays.asList(Schema.STRING_SCHEMA, Schema.INT32_SCHEMA)).newInstance(kaypherConfig);
    assertThat(substring1.evaluate("foo", 2), equalTo("oo"));

    final Kudf substring2 = function.getFunction(
        Arrays.asList(Schema.STRING_SCHEMA, Schema.INT32_SCHEMA, Schema.INT32_SCHEMA)).newInstance(kaypherConfig);
  ...
  public void shouldLoadUdafs() {
    final KaypherAggregateFunction instance = FUNC_REG
  ...
  public void shouldLoadDecimalUdfs() {
    ...
    // When:
    final KaypherScalarFunction fun = FUNC_REG.getUdfFactory("floor")
  ...
  public void shouldLoadFunctionWithListReturnType() {
    ...
    final KaypherScalarFunction function
  ...
  public void shouldLoadFunctionWithSchemaProvider() {
    ...
    final KaypherScalarFunction function = returnDecimal.getFunction(args);
  ...
  public void shouldThrowOnReturnTypeMismatch() {
    ...
    final Schema decimal = DecimalUtil.builder(2, 1).build();
    final List<Schema> args = Collections.singletonList(decimal);
    final KaypherScalarFunction function = returnIncompatible.getFunction(args);
  ...
  public void shouldThrowOnMissingAnnotation() throws ClassNotFoundException {
    ...
    final UdfClassLoader udfClassLoader = UdfClassLoader.newClassLoader(udfJar,
                                                                        PARENT_CLASS_LOADER,
                                                                        resourceName -> false);
    Class<?> clazz = udfClassLoader.loadClass("org.damian.kaypher.udf.MissingAnnotationUdf");
  ...
  public void shouldThrowOnMissingSchemaProvider() throws ClassNotFoundException {
    ...
    final UdfClassLoader udfClassLoader = UdfClassLoader.newClassLoader(udfJar,
                                                                        PARENT_CLASS_LOADER,
                                                                        resourceName -> false);
    Class<?> clazz = udfClassLoader.loadClass("org.damian.kaypher.udf.MissingSchemaProviderUdf");
    final UdfLoader udfLoader = new UdfLoader(
        functionRegistry,
        Optional.empty(),
        SqlTypeParser.create(TypeRegistry.EMPTY),
    ...
    expectedException.expectMessage(is("Cannot find schema provider method with name provideSchema "
                                           + "and parameter List<SqlType> in class org.damian.kaypher.udf."
  ...
  public void shouldThrowOnReturnDecimalWithoutSchemaProvider() throws ClassNotFoundException {
    ...
    final UdfClassLoader udfClassLoader = UdfClassLoader.newClassLoader(udfJar,
                                                                        PARENT_CLASS_LOADER,
                                                                        resourceName -> false);
    Class<?> clazz = udfClassLoader.loadClass("org.damian.kaypher.udf."
  ...
  public void shouldCreateUdfFactoryWithInternalPathWhenInternal() {
    ...
    assertThat(substring.getMetadata().getPath(), equalTo(KaypherScalarFunction.INTERNAL_PATH));
  }

  @Test
  public void shouldSupportUdfParameterAnnotation() {
    final UdfFactory substring = FUNC_REG.getUdfFactory("somefunction");
    final KaypherScalarFunction function = substring.getFunction(
        ImmutableList.of(
  ...
  @Test
  public void shouldPutKaypherFunctionsInParentClassLoader() throws Exception {
    final UdfFactory substring = FUNC_REG.getUdfFactory("substring");
  ...
  @Test
  public void shouldLoadUdfsInKAYPHERIfLoadCustomerUdfsFalse() {
    // udf in kaypher-engine will throw if not found
  ...
  public void shouldLoadSomeFunction() {
    ...
    final KaypherScalarFunction function = udfFactory.getFunction(args);
  ...
  public void shouldCollectMetricsWhenMetricCollectionEnabled() {
    ...
    final KaypherScalarFunction function = substring
        .getFunction(Arrays.asList(Schema.STRING_SCHEMA, Schema.INT32_SCHEMA));

    // When:
    final Kudf kudf = function.newInstance(kaypherConfig);

    // Then:
    assertThat(kudf, instanceOf(UdfMetricProducer.class));
    final Sensor sensor = METRICS.getSensor("kaypher-udf-substring");
    assertThat(sensor, not(nullValue()));
    assertThat(METRICS.metric(METRICS.metricName("kaypher-udf-substring-count", "kaypher-udf")),
        not(nullValue()));
    assertThat(METRICS.metric(METRICS.metricName("kaypher-udf-substring-max", "kaypher-udf")),
        not(nullValue()));
    assertThat(METRICS.metric(METRICS.metricName("kaypher-udf-substring-avg", "kaypher-udf")),
        not(nullValue()));
    assertThat(METRICS.metric(METRICS.metricName("kaypher-udf-substring-rate", "kaypher-udf")),
        not(nullValue()));
  }
  ...
  public void shouldUseConfigForExtDir() {
    ...
    final ImmutableMap<Object, Object> configMap
        = ImmutableMap.builder().put(KaypherConfig.KAYPHER_EXT_DIR, "src/test/resources")
        .put(KaypherConfig.KAYPHER_UDF_SECURITY_MANAGER_ENABLED, false)
        .build();
    final KaypherConfig config
        = new KaypherConfig(configMap);
  ...
  public void shouldNotThrowWhenExtDirDoesntExist() {
    final ImmutableMap<Object, Object> configMap
        = ImmutableMap.builder().put(KaypherConfig.KAYPHER_EXT_DIR, "foo/bar")
        .put(KaypherConfig.KAYPHER_UDF_SECURITY_MANAGER_ENABLED, false)
        .build();
    final KaypherConfig config
        = new KaypherConfig(configMap);
  ...
  public void shouldConfigureConfigurableUdfsOnInstantiation() {
    ...
    final KaypherConfig kaypherConfig = new KaypherConfig(ImmutableMap.of(
        KaypherConfig.KAYPHER_SERVICE_ID_CONFIG, "should not be passed",
        KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "configurableudf.some.setting", "foo-bar",
        KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "_global_.expected-param", "expected-value"
    ));

    final KaypherScalarFunction udf = FUNC_REG.getUdfFactory("ConfigurableUdf")
        .getFunction(ImmutableList.of(Schema.INT32_SCHEMA));

    // When:
    udf.newInstance(kaypherConfig);

    // Then:
    assertThat(PASSED_CONFIG, is(notNullValue()));
    assertThat(PASSED_CONFIG.keySet(), not(hasItem(KaypherConfig.KAYPHER_SERVICE_ID_CONFIG)));
    assertThat(PASSED_CONFIG.get(KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "configurableudf.some.setting"),
        is("foo-bar"));
    assertThat(PASSED_CONFIG.get(KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "_global_.expected-param"),
        is("expected-value"));
  }
  ...
  public void shouldEnsureFunctionReturnTypeIsOptional() throws Exception {
    ...
    // Then:
    final KaypherScalarFunction someFunction = FUNC_REG
  ...
  public void shouldEnsureFunctionReturnTypeIsDeepOptional() {
    final List<Schema> args = Collections.singletonList(Schema.OPTIONAL_STRING_SCHEMA);
    final KaypherScalarFunction complexFunction = FUNC_REG
  ...
  public void shouldImplementTableAggregateFunctionWhenTableUdafClass() throws Exception {
    final UdafFactoryInvoker creator
        ...
        "",
        "");
    final KaypherAggregateFunction function = creator
  ...
  public void shouldInvokeUdafWhenMethodHasArgs() throws Exception {
    final UdafFactoryInvoker creator
        ...
        "",
        "");
    final KaypherAggregateFunction instance =
  ...
  public void shouldCollectMetricsForUdafsWhenEnabled() throws Exception {
    ...
    final KaypherAggregateFunction<Long, Long, Long> executable =
    ...
    final KafkaMetric metric = metrics.metric(
        metrics.metricName("aggregate-test-udf-createSumLong-count",
            "kaypher-udaf-test-udf-createSumLong"));
  ...
  @Test
  public void shouldThrowKaypherFunctionExceptionIfNullPassedWhenExpectingPrimitiveType()
      throws Exception {
    expectedException.expect(KaypherFunctionException.class);
    expectedException
  ...
  public void shouldThrowWhenUdafReturnTypeIsntAUdaf() throws Exception {
    expectedException.expect(KaypherException.class);
    expectedException.expectMessage("UDAFs must implement com.treutec.kaypher.function.udaf.Udaf or com.treutec.kaypher.function.udaf.TableUdaf. method='createBlah', functionName='`test`', UDFClass='class com.treutec.kaypher.function.UdfLoaderTest");
    createUdafLoader().createUdafFactoryInvoker(
  ...
  public void shouldThrowWhenUdafFactoryMethodIsntStatic() throws Exception {
    ...
    // Expect:
    expectedException.expect(KaypherException.class);
    expectedException.expectMessage("UDAF factory methods must be static public com.treutec.kaypher.function.udaf.Udaf");
