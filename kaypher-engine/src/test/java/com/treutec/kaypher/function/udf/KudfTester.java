...
public class KudfTester {
   ...
   * return in a {@link KaypherException} being thrown.
   *
    ...
    for (int idx = 0; idx != theArgs.size(); ++idx) {
      ...
      try {
        ...
        if (exceptionExpected) {
          fail("Expected UDF to throw KaypherException when arg " + idx + " is null");
        }
      } catch (final KaypherException e) {
        if (!exceptionExpected) {
        ...
        if (exceptionExpected) {
          throw new AssertionError("Unexpected exception thrown by UDF when arg "
                                   + idx + " is null. Expected KaypherException", e);
        }
        throw new AssertionError("Unexpected exception thrown by UDF when arg "
                                 + idx + " is null.", e);
      }
    ...
    for (int idx = 0; idx != theArgs.size(); ++idx) {
      ...
      try {
        ...
        throw new AssertionError("UDF threw unexpected exception type when arg " + idx
                                 + " was the wrong type. Expected: KAYPHERException", e);
      }
