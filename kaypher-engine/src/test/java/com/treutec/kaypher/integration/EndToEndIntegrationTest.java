...
import com.treutec.kaypher.util.PageViewDataProvider;
 ...
 * are what we expect. This tests a broad set of KAYPHER functionality and is a good catch-all.
 */
...
public class EndToEndIntegrationTest {
  ...
  public final TestKaypherContext kaypherContext = TEST_HARNESS.kaypherContextBuilder()
      ...
      .withAdditionalConfig(
          KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "e2econfigurableudf.some.setting",
          "foo-bar"
      )
      .withAdditionalConfig(
          KAYPHER_FUNCTIONS_PROPERTY_PREFIX + "_global_.expected-param",
          "expected-value"
      )
      .build();
  ...
  public void before() {
    ...
    kaypherContext.sql("CREATE STREAM " + PAGE_VIEW_STREAM
  ...
  public void shouldSupportDroppingAndRecreatingJoinQuery() throws Exception {
    ...
    kaypherContext.terminateQuery(new QueryId("CSAS_CART_EVENT_PRODUCT_0"));
  ...
  public void shouldCleanUpAvroSchemaOnDropSource() throws Exception {
    ...
    TEST_HARNESS.waitForSubjectToBePresent(topicName + KaypherConstants.SCHEMA_REGISTRY_VALUE_SUFFIX);

    kaypherContext.terminateQuery(new QueryId("CSAS_AVRO_STREAM_0"));

    executeStatement("DROP STREAM avro_stream DELETE TOPIC;");

    TEST_HARNESS.waitForSubjectToBeAbsent(topicName + KaypherConstants.SCHEMA_REGISTRY_VALUE_SUFFIX);
  }
  ...
  private <T extends QueryMetadata> T executeStatement(
    ...
    final List<QueryMetadata> queries = kaypherContext.sql(formatted);
