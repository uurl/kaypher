...
import static com.treutec.kaypher.test.util.ConsumerTestUtil.toUniqueRecords;
import static com.treutec.kaypher.test.util.MapMatchers.mapHasSize;
...
import com.treutec.kaypher.test.util.ConsumerTestUtil;
...
import com.treutec.kaypher.util.TestDataProvider;
...
public final class IntegrationTestHarness extends ExternalResource {
  ...
  public TestKaypherContext buildKaypherContext() {
    return kaypherContextBuilder().build();
  }

  public ContextBuilder kaypherContextBuilder() {
  ...
  public void ensureSchema(
    ...
    try {
      final org.apache.avro.Schema avroSchema = AvroSchemas
          .getAvroSchema(schema.valueSchema(), "test_" + topicName, kaypherConfig);

      srClient.register(topicName + KaypherConstants.SCHEMA_REGISTRY_VALUE_SUFFIX, avroSchema);
    } catch (final Exception e) {
  ...
  public final class ContextBuilder {
    ...
    public TestKaypherContext build() {
      return new TestKaypherContext(IntegrationTestHarness.this, additionalConfig);
    }
  }
    ...
    private ServiceContext get() {
      if (serviceContext.get() == null) {
        final ServiceContext created = TestServiceContext.create(
            KaypherConfigTestUtil.create(kafkaCluster),
