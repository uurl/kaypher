...
public class UdfIntTest {
  ...
  @Before
  public void before() {
    intermediateStream = KaypherIdentifierTestUtil.uniqueIdentifierName("INTERMEDIATE");
    resultStreamName = KaypherIdentifierTestUtil.uniqueIdentifierName("OUTPUT");
  ...
  private void createSourceStream() {
    if (testData.format == DELIMITED) {
      ...
      kaypherContext.sql(String.format("CREATE STREAM %s (ID varchar, DESCRIPTION varchar) WITH "
              + "(kafka_topic='%s', value_format='%s');",
          testData.sourceStreamName, testData.sourceTopicName, testData.format.name()));
    } else {
      kaypherContext.sql(String.format("CREATE STREAM %s ("
              + "ORDERTIME bigint, "
              + "ORDERID varchar, "
              + "ITEMID varchar, "
              + "ORDERUNITS double, "
