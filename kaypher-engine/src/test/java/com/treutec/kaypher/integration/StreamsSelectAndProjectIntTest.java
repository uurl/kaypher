...
public class StreamsSelectAndProjectIntTest {
  ...
  @Before
  public void before() {
    intermediateStream = KaypherIdentifierTestUtil.uniqueIdentifierName("int");
    resultStream = KaypherIdentifierTestUtil.uniqueIdentifierName("output");
  ...
  public void shouldSkipBadData() {
    kaypherContext.sql("CREATE STREAM " + intermediateStream + " AS"
  ...
  public void shouldUseStringTimestampWithFormat() throws Exception {
    kaypherContext.sql("CREATE STREAM " + intermediateStream +
  ...
  public void shouldUseTimestampExtractedFromDDLStatement() throws Exception {
    kaypherContext.sql("CREATE STREAM " + resultStream + " WITH(timestamp='ordertime')"
  ...
  private void testTimestampColumnSelection(
    ...
    kaypherContext.sql(query1String);
  ...
  private void testSelectProjectKeyTimestamp(
    ...
    kaypherContext.sql(String.format("CREATE STREAM %s AS SELECT ROWKEY AS RKEY, ROWTIME "
  ...
  private void testSelectProject(
    ...
    kaypherContext.sql(String.format("CREATE STREAM %s AS SELECT ITEMID, ORDERUNITS, "
  ...
  public void testSelectProjectAvroJson() {
    ...
    kaypherContext.sql(String.format("CREATE STREAM %s WITH ( value_format = 'JSON') AS SELECT "
                                  + "ITEMID, "
                                  + "ORDERUNITS, "
  ...
  private void testSelectStar(
    ...
    kaypherContext.sql(String.format("CREATE STREAM %s AS SELECT * FROM %s;",
                                  resultStream,
                                  inputStreamName));
  ...
  private void testSelectWithFilter(
    ...
    kaypherContext.sql("CREATE STREAM " + resultStream + " AS "
  ...
  public void testInsertIntoJson() {
    ...
    kaypherContext.sql("INSERT INTO " + resultStream +
  ...
  public void testInsertSelectStarJson() {
    ...
    kaypherContext.sql("INSERT INTO " + resultStream + " SELECT * FROM " + JSON_STREAM_NAME + ";");
  ...
  public void testInsertSelectStarAvro() {
    ...
    kaypherContext.sql("INSERT INTO " + resultStream + " SELECT * FROM " + AVRO_STREAM_NAME + ";");
  ...
  private PhysicalSchema getResultSchema() {
    final DataSource<?> source = kaypherContext
        .getMetaStore()
  ...
  private void createOrdersStream() {
    ...
    kaypherContext.sql("CREATE STREAM " + JSON_STREAM_NAME + " (" + columns + ") WITH "
        + "(kafka_topic='" + jsonTopicName + "', value_format='JSON', key='ordertime');");

    kaypherContext.sql("CREATE STREAM " + AVRO_STREAM_NAME + " (" + columns + ") WITH "
        + "(kafka_topic='" + avroTopicName + "', value_format='AVRO', key='ordertime');");

    kaypherContext.sql("CREATE STREAM " + AVRO_TIMESTAMP_STREAM_NAME + " (" + columns + ") WITH "
        + "(kafka_topic='" + avroTopicName + "', value_format='AVRO', key='ordertime', "
