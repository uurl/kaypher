...
public class AggregateNodeTest {
  ...
  private static final KaypherConfig KAYPHER_CONFIG =  new KaypherConfig(new HashMap<>());
  ...
  @Test
  public void shouldHaveSourceNodeForSecondSubtopolgyWithKaypherNameForRepartition() {
    // When:
    buildRequireRekey();
  ...
  @Test
  public void shouldHaveKaypherNameForAggregationStateStore() {
    build();
    final TopologyDescription.Processor node = (TopologyDescription.Processor) getNodeByName(
  ...
  public void shouldBuildCorrectAggregateSchema() {
    ...
    assertThat(stream.getSchema().value(), contains(
        ...
        Column.of(ColumnName.of("KAYPHER_COL_1"), SqlTypes.DOUBLE),
        Column.of(ColumnName.of("KAYPHER_COL_2"), SqlTypes.BIGINT)));
  }
  ...
  private SchemaKStream build() {
    return build(KAYPHER_CONFIG);
  }

  private SchemaKStream build(final KaypherConfig kaypherConfig) {
    return buildQuery("SELECT col0, sum(col3), count(col3) FROM test1 window TUMBLING ( "
        + "size 2 "
        + "second) "
        + "WHERE col0 > 100 GROUP BY col0 EMIT CHANGES;", kaypherConfig);
  }

  @SuppressWarnings("UnusedReturnValue")
  private SchemaKStream buildRequireRekey() {
    return buildRequireRekey(KAYPHER_CONFIG);
  }

  @SuppressWarnings("UnusedReturnValue")
  private SchemaKStream buildRequireRekey(final KaypherConfig kaypherConfig) {
    return buildQuery("SELECT col1, sum(col3), count(col3) FROM test1 window TUMBLING ( "
        + "size 2 "
        + "second) "
        + "GROUP BY col1 EMIT CHANGES;", kaypherConfig);
  }
  ...
  public void shouldCreateLoggers() {
    ...
    buildQuery(node, KAYPHER_CONFIG);

    // Then:
    verify(kaypherStreamBuilder, times(3)).buildValueSerde(
        any(),
        any(),
  ...
  private SchemaKStream buildQuery(final String queryString) {
    return buildQuery(queryString, KAYPHER_CONFIG);
  }

  private SchemaKStream buildQuery(final String queryString, final KaypherConfig kaypherConfig) {
    return buildQuery(buildAggregateNode(queryString), kaypherConfig);
  }

  private SchemaKStream buildQuery(final AggregateNode aggregateNode, final KaypherConfig kaypherConfig) {
    ...
    when(kaypherStreamBuilder.getProcessingLogContext()).thenReturn(processingLogContext);
    when(kaypherStreamBuilder.getFunctionRegistry()).thenReturn(FUNCTION_REGISTRY);
    ...
    final SchemaKTable schemaKTable = (SchemaKTable) aggregateNode.buildStream(kaypherStreamBuilder);
    schemaKTable.getSourceTableStep().build(new KSPlanBuilder(kaypherStreamBuilder));
    return schemaKTable;
  }
  ...
  private static AggregateNode buildAggregateNode(final String queryString) {
    ...
    final KaypherBareOutputNode planNode = (KaypherBareOutputNode) AnalysisTestUtil
        .buildLogicalPlan(KAYPHER_CONFIG, queryString, newMetaStore);
