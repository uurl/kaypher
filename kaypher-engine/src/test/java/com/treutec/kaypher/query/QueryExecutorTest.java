...
import com.treutec.kaypher.logging.processing.ProcessingLoggerFactory;
...
public class QueryExecutorTest {
  private static final String STATEMENT_TEXT = "KAYPHER STATEMENT";
  ...
  @Mock
  private KaypherMaterializationFactory kaypherMaterializationFactory;
  @Mock
  ...
  public void setup() {
    ...
    when(sink.getKaypherTopic()).thenReturn(kaypherTopic);
    when(sink.getName()).thenReturn(SINK_NAME);
    when(sink.getDataSourceType()).thenReturn(DataSourceType.KSTREAM);
    when(kaypherTopic.getKeyFormat()).thenReturn(KEY_FORMAT);
    ...
    when(kaypherMaterializationFactory.create(any(), any(), any(), any())).thenReturn(materialization);
    when(processingLogContext.getLoggerFactory()).thenReturn(processingLoggerFactory);
    when(processingLoggerFactory.getLogger(any())).thenReturn(processingLogger);
    when(kaypherConfig.getKaypherStreamConfigProps()).thenReturn(Collections.emptyMap());
    when(kaypherConfig.getString(KaypherConfig.KAYPHER_PERSISTENT_QUERY_NAME_PREFIX_CONFIG))
        .thenReturn(PERSISTENT_PREFIX);
    when(kaypherConfig.getString(KaypherConfig.KAYPHER_SERVICE_ID_CONFIG)).thenReturn(SERVICE_ID);
  ...
  public void shouldBuildPersistentQueryCorrectly() {
    ...
    assertThat(queryMetadata.getResultTopic(), is(kaypherTopic));
    assertThat(queryMetadata.getSourceNames(), equalTo(SOURCES));
  ...
  public void shouldMaterializeCorrectly() {
    ...
    // Then:
    verify(kaypherMaterializationFactory).create(
        ksMaterialization,
        materializationInfo,
        QUERY_ID,
        stacker
  ...
  public void shouldCreateExpectedServiceId() {
    ...
    assertThat(
        ...
        equalTo("_confluent-kaypher-service-persistent-queryid")
    );
  }
  ...
  private void shouldUseProvidedOptimizationConfig(final Object value) {
    ...
    when(kaypherConfig.getKaypherStreamConfigProps()).thenReturn(properties);
  ...
  public void shouldAddMetricsInterceptorsToExistingList() {
    // Given:
    when(kaypherConfig.getKaypherStreamConfigProps()).thenReturn(ImmutableMap.of(
        StreamsConfig.consumerPrefix(ConsumerConfig.INTERCEPTOR_CLASSES_CONFIG),
  ...
  public void shouldConfigureProducerErrorHandler() {
    ...
    assertThat(
        streamsProps.get(ProductionExceptionHandlerUtil.KAYPHER_PRODUCTION_ERROR_LOGGER),
        is(logger));
  }
