...
public class DefaultSchemaInjectorFunctionalTest {
  ...
  public void shouldInferComplexConnectSchema() {
    ...
    final Schema kaypherArrayInner = SchemaBuilder.struct()
        .field("ARRAYINNER1", Schema.OPTIONAL_STRING_SCHEMA)
        .field("ARRAYINNER2", Schema.OPTIONAL_INT64_SCHEMA)
        .optional()
        .build();
    final Schema kaypherMapInner = SchemaBuilder.struct()
        .field("MAPINNER1", Schema.OPTIONAL_FLOAT64_SCHEMA)
        .field("MAPINNER2", Schema.OPTIONAL_STRING_SCHEMA)
        .optional()
        .build();
    final Schema kaypherStructInner2 = SchemaBuilder.struct()
        .field("STRUCTINNER2_1", Schema.OPTIONAL_BOOLEAN_SCHEMA)
        .field("STRUCTINNER2_2", Schema.OPTIONAL_INT32_SCHEMA)
        .optional()
        .build();
    final Schema kaypherStructInner1 = SchemaBuilder.struct()
        .field("STRUCTINNER1_1", Schema.OPTIONAL_STRING_SCHEMA)
        .field("STRUCTINNER1_2", kaypherStructInner2)
        .optional()
        .build();
    final Schema kaypherSchema = SchemaBuilder.struct()
        .field("PRIMITIVE", Schema.OPTIONAL_INT32_SCHEMA)
        .field("ARRAY", kaypherArrayInner)
        .field("MAP", kaypherMapInner)
        .field("STRUCT", kaypherStructInner1)
        .build();
  ...
  private void shouldInferType(
      ...
      final Schema expectedKaypherSchema
  ) {
    ...
    final SchemaBuilder kaypherStreamSchemaBuilder = SchemaBuilder.struct();
    if (expectedKaypherSchema != null) {
      kaypherStreamSchemaBuilder.field("FIELD0", expectedKaypherSchema);
    }

    shouldInferSchema(avroStreamSchema, kaypherStreamSchemaBuilder.build());
  }
  ...
  private void shouldInferSchema(
    ...
    final PreparedStatement<Statement> prepared = KaypherParserTestUtil
        .buildSingleAst(stmtNoSchema, metaStore);

    // When:
    final ConfiguredStatement<?> inferred = schemaInjector.inject(
        ConfiguredStatement.of(prepared, ImmutableMap.of(), new KaypherConfig(ImmutableMap.of())));

    // Then:
    final Statement withSchema = KaypherParserTestUtil
