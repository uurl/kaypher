...
public class TopicCreateInjectorTest {
  ...
  @Before
  public void setUp() {
    parser = new DefaultKaypherParser();
    metaStore = new MetaStoreImpl(new InternalFunctionRegistry());
    overrides = new HashMap<>();
    config = new KaypherConfig(new HashMap<>());
    ...
    final KaypherStream source = new KaypherStream<>(
        "",
    ...
    final KaypherTopic joinTopic = new KaypherTopic(
        "jSource",
        KeyFormat.nonWindowed(FormatInfo.of(Format.KAFKA)),
        ValueFormat.of(FormatInfo.of(Format.JSON)),
        false
    ...
    final KaypherStream joinSource = new KaypherStream<>(
        "",
    ...
    when(builder.withKaypherConfig(any())).thenReturn(builder);
  ...
  public void shouldGenerateNameWithCorrectPrefixFromOverrides() {
    // Given:
    overrides.put(KaypherConfig.KAYPHER_OUTPUT_TOPIC_NAME_PREFIX_CONFIG, "prefix-");
    givenStatement("CREATE STREAM x AS SELECT * FROM SOURCE;");
    config = new KaypherConfig(ImmutableMap.of(
        KaypherConfig.KAYPHER_OUTPUT_TOPIC_NAME_PREFIX_CONFIG, "nope"
    ));
  ...
  public void shouldGenerateNameWithCorrectPrefixFromConfig() {
    ...
    givenStatement("CREATE STREAM x AS SELECT * FROM SOURCE;");
    config = new KaypherConfig(ImmutableMap.of(
        KaypherConfig.KAYPHER_OUTPUT_TOPIC_NAME_PREFIX_CONFIG, "prefix-"
    ));
  ...
  public void shouldPassThroughConfigToBuilder() {
    ...
    // Then:
    verify(builder).withKaypherConfig(config);
  }
