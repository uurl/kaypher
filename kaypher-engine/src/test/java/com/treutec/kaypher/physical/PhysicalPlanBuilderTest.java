...
import static com.treutec.kaypher.metastore.model.MetaStoreMatchers.hasSerdeOptions;
...
public class PhysicalPlanBuilderTest {
  ...
  private static final KaypherConfig INITIAL_CONFIG = KaypherConfigTestUtil.create("what-eva");
  ...
  public void shouldCreateExecutionPlan() {
    ...
    assertThat(lines[0], startsWith(
        " > [ PROJECT ] | Schema: [ROWKEY STRING KEY, COL0 BIGINT, KAYPHER_COL_1 DOUBLE, "
            + "KAYPHER_COL_2 BIGINT] |"));
    assertThat(lines[1], startsWith(
        "\t\t > [ AGGREGATE ] | Schema: [ROWKEY STRING KEY, KAYPHER_INTERNAL_COL_0 BIGINT, "
            + "KAYPHER_INTERNAL_COL_1 DOUBLE, KAYPHER_AGG_VARIABLE_0 DOUBLE, "
            + "KAYPHER_AGG_VARIABLE_1 BIGINT] |"));
    assertThat(lines[2], startsWith(
        "\t\t\t\t > [ PROJECT ] | Schema: [ROWKEY STRING KEY, KAYPHER_INTERNAL_COL_0 BIGINT, "
            + "KAYPHER_INTERNAL_COL_1 DOUBLE] |"));
  ...
  public void shouldCreateExecutionPlanForInsert() {
    ...
    Assert.assertEquals(lines[2],
        "\t\t\t\t > [ SOURCE ] | Schema: [TEST1.ROWKEY STRING KEY, TEST1.ROWTIME BIGINT, TEST1.ROWKEY STRING, TEST1.COL0 BIGINT, TEST1.COL1 STRING, TEST1.COL2 DOUBLE] | Logger: InsertQuery_1.KaypherTopic.source");
  ...
  public void shouldGetSingleValueSchemaWrappingFromPropertiesBeforeConfig() {
    // Given:
    givenConfigWith(KaypherConfig.KAYPHER_WRAP_SINGLE_VALUES, true);
    givenKafkaTopicsExist("test1");
  ...
  public void shouldGetSingleValueSchemaWrappingFromConfig() {
    // Given:
    givenConfigWith(KaypherConfig.KAYPHER_WRAP_SINGLE_VALUES, false);
  ...
  private void givenConfigWith(final String name, final Object value) {
    kaypherConfig = kaypherConfig.cloneWithPropertyOverwrite(ImmutableMap.of(name, value));
  }

  private List<QueryMetadata> execute(final String sql) {
    return KaypherEngineTestUtil.execute(
        serviceContext,
        kaypherEngine,
        sql,
        kaypherConfig,
