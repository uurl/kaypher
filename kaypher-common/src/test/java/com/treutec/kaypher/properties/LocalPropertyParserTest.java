...
public class LocalPropertyParserTest {
  ...
  public void shouldNotCallResolverForRunScriptConstant() {
    // When:
    parser.parse(KaypherConstants.LEGACY_RUN_SCRIPT_STATEMENTS_CONTENT, "100");
  ...
  public void shouldCallValidatorForRunScriptConstant() {
    // When:
    parser.parse(KaypherConstants.LEGACY_RUN_SCRIPT_STATEMENTS_CONTENT, "something2");

    // Then:
    verify(validator).validate(KaypherConstants.LEGACY_RUN_SCRIPT_STATEMENTS_CONTENT, "something2");
  }

  @Test
  public void shouldCallResolverForOtherProperties() {
    // When:
    parser.parse(KaypherConfig.KAYPHER_SERVICE_ID_CONFIG, "100");

    // Then:
    verify(resolver).resolve(KaypherConfig.KAYPHER_SERVICE_ID_CONFIG, true);
  }
  ...
  public void shouldThrowIfResolverFailsToResolve() {
    ...
    expectedException.expectMessage(
        "Not recognizable as kaypher, streams, consumer, or producer property: 'Unknown'");
