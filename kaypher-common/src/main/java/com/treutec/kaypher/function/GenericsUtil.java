...
public final class GenericsUtil {
   ...
   *
   * @throws KaypherException if there is a generic in {@code schema} that is not present
   *                       in {@code mapping}
   */
  public static Schema applyResolved(final Schema schema, final Map<Schema, Schema> resolved) {
    switch (schema.type()) {
      ...
      case BYTES:
        ...
        if (instance == null) {
          throw new KaypherException("Could not find mapping for generic type: " + schema);
        }
        return instance;
      default:
        return schema;
  ...
  private static boolean resolveGenerics(
    ...
    switch (schema.type()) {
      ...
      case STRUCT:
        throw new KaypherException("Generic STRUCT is not yet supported");
      default:
        return true;
    }
  }
   ...
   * @throws KaypherException if {@code schema} is not a generic schema
   */
  public static String name(final Schema schema) {
    ...
    throw new KaypherException("Cannot extract name from non-generic schema: " + schema);
  }
}
