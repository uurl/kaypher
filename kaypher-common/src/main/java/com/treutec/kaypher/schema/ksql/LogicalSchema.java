...
import com.treutec.kaypher.schema.kaypher.SchemaConverters.SqlToConnectTypeConverter;
...
/**
 * Immutable KAYPHER logical schema.
 */
@Immutable
  ...
  public static class Builder {
    ...
    public Builder keyColumn(final Column column) {
      if (!seenKeys.add(column.ref())) {
        throw new KaypherException("Duplicate keys found in schema: " + column);
      }
    ...
    public Builder valueColumn(final Column column) {
      if (!seenValues.add(column.ref())) {
        throw new KaypherException("Duplicate values found in schema: " + column);
      }
