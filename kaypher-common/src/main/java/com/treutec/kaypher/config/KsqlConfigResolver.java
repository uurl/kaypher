...
/**
 * Resolves Kaypher and streams property name to a ConfigItem.
 */
public class KaypherConfigResolver implements ConfigResolver {
  ...
  public  Optional<ConfigItem> resolve(final String propertyName, final boolean strict) {
    if (propertyName.startsWith(KaypherConfig.KAYPHER_CONFIG_PROPERTY_PREFIX)
        && !propertyName.startsWith(KaypherConfig.KAYPHER_STREAMS_PREFIX)) {
      return resolveKaypherConfig(propertyName);
    }
  ...
  private static Optional<ConfigItem> resolveStreamsConfig(
    ...
    final String key = stripPrefix(propertyName, KaypherConfig.KAYPHER_STREAMS_PREFIX);
    ...
    if (propertyName.startsWith(KaypherConfig.KAYPHER_STREAMS_PREFIX)) {
  ...
  private static Optional<ConfigItem> resolveKaypherConfig(final String propertyName) {
    final Optional<ConfigItem> possibleItem = resolveConfig("", KAYPHER_CONFIG_DEF, propertyName);
    if (possibleItem.isPresent()) {
      return possibleItem;
    }

    if (propertyName.startsWith(KaypherConfig.KAYPHER_FUNCTIONS_PROPERTY_PREFIX)) {
