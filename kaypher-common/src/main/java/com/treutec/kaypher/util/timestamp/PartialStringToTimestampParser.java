...
import static com.treutec.kaypher.util.KaypherConstants.TIME_PATTERN;
...
public class PartialStringToTimestampParser {
  ...
  private static final String HELP_MESSAGE = System.lineSeparator()
      + "Required format is: \"" + KaypherConstants.DATE_TIME_PATTERN + "\", "
  ...
  private static final StringToTimestampParser PARSER =
      new StringToTimestampParser(KaypherConstants.DATE_TIME_PATTERN);
  ...
  public long parse(final String text) {
    ...
    try {
      ...
      throw new KaypherException("Failed to parse timestamp '" + text
          + "': " + e.getMessage()
          + HELP_MESSAGE,
          e
      );
  ...
  private static ZoneId parseTimezone(final String timezone) {
    ...
    try {
      ...
      throw new KaypherException("Failed to parse timezone '" + timezone
          + "': " + e.getMessage(),
          e
      );
    }
