...
public final class DecimalUtil {
  ...
  /**
   * @param schema the schema to test
   * @throws KaypherException if the schema does not match {@link #isDecimal(Schema)}
   */
   ...
   * @throws KaypherException if the value does not fit
   */
   ...
   * @throws KaypherException if the schema cannot safely be converted to decimal
   */
  public static SqlDecimal toSqlDecimal(final Schema schema) {
    switch (schema.type()) {
      ...
      default:
        throw new KaypherException("Cannot convert schema of type " + schema.type() + " to decimal.");
    }
  }
  ...
  public static void validateParameters(final int precision, final int scale) {
    KaypherPreconditions.checkArgument(precision > 0,
        String.format("DECIMAL precision must be >= 1: DECIMAL(%d,%d)", precision, scale));
    KaypherPreconditions.checkArgument(scale >= 0,
        String.format("DECIMAL scale must be >= 0: DECIMAL(%d,%d)", precision, scale));
    KaypherPreconditions.checkArgument(precision >= scale,
