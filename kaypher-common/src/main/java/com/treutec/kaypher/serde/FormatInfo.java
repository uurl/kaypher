...
public final class FormatInfo {
  ...
  private FormatInfo(
    ...
    if (format != Format.AVRO && avroFullSchemaName.isPresent()) {
      throw new KaypherException("Full schema name only supported with AVRO format");
    }

    if (format == Format.AVRO
        && avroFullSchemaName.map(name -> name.trim().isEmpty()).orElse(false)) {
      throw new KaypherException("Schema name cannot be empty");
    }

    this.delimiter = Objects.requireNonNull(delimiter, "delimiter");

    if (format != Format.DELIMITED && delimiter.isPresent()) {
      throw new KaypherException("Delimeter only supported with DELIMITED format");
    }
